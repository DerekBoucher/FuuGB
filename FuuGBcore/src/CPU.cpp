...
{
	if (checkCarryFromBit_Byte(4, host, operand + carry))
		CPU_FLAG_BIT_SET(H_FLAG);
	else
		CPU_FLAG_BIT_RESET(H_FLAG);
...
void CPU::updateTimers(int cycles)
{
	...
	if (TMC.test(2)) //Check if clock is enabled
	{
		...
		if (memory->timer_counter <= 0)
		{
			...
			{
			case 0:
				memory->timer_counter = 1024;
				break;
			case 1:
				memory->timer_counter = 16;
				break;
			case 2:
				memory->timer_counter = 64;
				break;
			case 3:
				memory->timer_counter = 256;
				break;
			}
